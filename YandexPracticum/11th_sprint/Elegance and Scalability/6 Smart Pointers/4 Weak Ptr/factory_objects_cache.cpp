...
public:
    ...
    explicit Cache(ValueFactoryFn value_factory) {
        chacher = std::move(value_factory);
    }
    ...
    std::shared_ptr<Value> GetValue(const Key& key) {
        ...
        {
            std::shared_ptr<Value> temp = chacher(key);
            storage[key] = temp;
            return temp;

        }
        else if (storage.at(key).expired())
        {
            std::shared_ptr<Value> temp = chacher(key);
            storage[key] = temp;
            return temp;
...
public:
    ...
    {
        using namespace std;
        cout << "Object '" << id_ << "' has been created" << endl;
    }
    ...
    ~Object() {
        using namespace std;
        cout << "Object '" << id_ << "' has been destroyed" << endl;
    }
...
public:
    ...
    explicit BookLoader(const BookStore& store, size_t& load_count) :list_(&store), count_(&load_count) {
    ...
    std::shared_ptr<Book> operator()(const std::string& title) const {
        if (list_->count(title) > 0)
        {
            (*count_)++;
            return std::make_shared<Book>(title, list_->at(title));
        }
        else
        {
            throw std::out_of_range("no such book in the library");
        }

    }

private:
    const std::unordered_map<std::string, std::string>* list_;
    size_t* count_;
};
...
void Test2() {
    ...
    {"Harry Potter"s, "Chapter 1. The boy who lived. ...."},
    };
