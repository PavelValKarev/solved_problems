...
public:
    ...
    //из всех сеттеров будем возвращать ссылку на
    //текущий объект билдера
...
int main() {
    //Можно объединить цепочки вызовов сеттеров
    //ReadingManagerBuilder builder можно создать
    //даже как временную переменную без названия
    //И результат вызова этого выражения можно
    //присвоить в объект класса ReadingManager
    ReadingManager manager = ReadingManagerBuilder()
            .SetMaxUserCount(100000)
            .SetMaxPageCount(1000)
            .SetCheerFactor(2)
            .Build();
    /*
    Итого в этом коде написано следующее: Я хочу
    создать ReadingManager. Для этого я беру билдер,
    вызываю у него сеттеры, вызываю м-д Build() И
    получаю свой ReadingManager()
    */
    /*
    Паттерн проектирования Builder() позволяет
    разграничить инициализацию класса и его 
